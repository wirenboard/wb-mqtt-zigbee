...
На схеме ниже показано движение данных через mqtt брокер (mosquitto): слева — пространство топиков zigbee2mqtt, справа — пространство контроллера WirenBoard. Стрелки двунаправленные: состояния устройств идут слева направо, команды — справа налево.
...
> - оба пространства топиков живут в одном MQTT-брокере (mosquitto).
> - cписок устройств (`bridge/devices`) публикуется как *retained* один раз при старте zigbee2mqtt.


## Эксплуатация

Сервис работает как systemd-юнит `wb-mqtt-zigbee` (`Restart=on-failure`, `RestartPreventExitStatus=2 6`), журнал — `journalctl -u wb-mqtt-zigbee`. Коды выхода:

| Код | Причина | Что делает systemd |
|---|---|---|
| 0 | штатная остановка по SIGINT/SIGTERM/SIGHUP | не перезапускает |
| 1 | сбой в работе (необработанная ошибка) | перезапускает через 10 с |
| 2 | брокер отверг логин MQTT (неверные учётные данные или нет прав) — при старте или при переподключении | не перезапускает, пока не исправят учётные данные и не запустят сервис вручную |
| 6 | ошибка конфигурации: файл не читается, не JSON, неверный адрес брокера | не перезапускает, пока не исправят конфиг и не запустят сервис вручную |

Код 7 («не настроен, не запускать») этот сервис не использует.

Остановка wb-mqtt-zigbee снимает все его retained-топики: Zigbee-устройства исчезают из интерфейса WB и из правил, привязанных к их топикам, при этом zigbee2mqtt продолжает работать и управлять Zigbee-сетью, а после запуска сервиса устройства появляются снова.
