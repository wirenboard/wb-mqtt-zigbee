...
Имена устройств (`friendly_name`) используются как сегменты MQTT-топиков. Для защиты от подписки на wildcard-топики и инъекции лишнего уровня топика проверяется отсутствие символов `+`, `#` и `/` (а также пустого имени) в `friendly_name`. Проверка применяется не только при первичной регистрации, но и на событии переименования (`bridge/event` `device_renamed`) и перед любой per-device подпиской/публикацией (`subscribe`/`get`/`set`). Устройства с небезопасными именами пропускаются, а небезопасное переименование отклоняется — со предупреждением в лог.
...
- **Форма и тип**: `_parse_json_payload` отвергает JSON неожиданного верхнего типа (число/строка/массив там, где ждём объект). Список `bridge/devices` дополнительно отбрасывается целиком, если это не массив объектов с `ieee_address` — иначе `[{}]` спровоцировал бы stale-removal и снёс все реальные устройства.
...
При парсинге списка устройств из `bridge/devices` ошибка в одном устройстве (невалидный JSON, отсутствие полей, несовместимые типы полей) не блокирует обработку остальных: `_register_device` обёрнут в `try/except`, ошибка логируется с трейсбеком, остальные устройства регистрируются нормально, а stale/ghost-очистка после цикла всё равно отрабатывает.
