...
- Python 3.9
- `python3-wb-common (>= 2.4.0)` — единственная runtime-зависимость (включает paho-mqtt); 2.4.0 нужна для `start(retry_first_connection=True)` и `wait_for_connection()`
...
on_connect (реконнект):
  → bridge.republish()
    → перепубликует meta и контролы моста, заново подписывается на z2m-топики и WB-команды моста
    → инкрементирует счётчик Reconnects
    → каждое известное устройство публикует заново целиком (meta + контролы) с последними
      опубликованными значениями контролов (publisher.last_values), но с available=offline
      и meta/error — пока zigbee2mqtt не подтвердит его (availability или свежий state)
    → сбрасывает availability_received для восстановления fallback
    → заново подписывается на /on-команды устройства и на его z2m-топик, запрашивает state
    → перезапрашивает bridge/devices
```
...
Исключения в коллбэках MQTT обрабатываются точечно, а не глобальным `suppress_exceptions`. Каждый message-коллбэк (обработчики входящих сообщений, регистрируемые через `message_callback_add`) оборачивается в `log_callback_errors`: ошибка на битом или неожиданном сообщении логируется с полным трейсбеком и топиком, а цикл paho продолжает работать — само сообщение потеряно, корректные данные придут со следующим. Lifecycle-коллбэки (`on_connect`/`on_disconnect`) намеренно **не** оборачиваются: сбой при старте (например, в `subscribe()`) должен закончиться ненулевым кодом `run()`, чтобы systemd перезапустил сервис, а не оставил его «живым, но бездействующим». Цикл идёт в сетевом потоке paho, и исключение, вылетевшее оттуда, только убило бы этот поток, поэтому `on_connect` ловит сбой сам, пишет трейсбек и передаёт его главному потоку (код выхода 1 и запрос остановки); тот снимает наши retained-топики и останавливает клиента. Так же устроена и штатная остановка: обработчик SIGINT/SIGTERM/SIGHUP лишь взводит событие, а главный поток снимает топики, ждёт подтверждения последнего снятия не дольше `REMOVAL_TIMEOUT_S` (10 с, заметно меньше 90 с systemd) и зовёт `stop()`; не подтверждённое снятие даёт строку в журнале и принудительный `disconnect()`, чтобы поток paho мог выйти. Глобальный `suppress_exceptions` эту разницу стирал бы — он глушит и lifecycle-сбой, и прячет трейсбек за одной строкой.
...
- python3.9
- python3-wb-common (>= 2.4.0) — включает paho-mqtt
...
`wb_common.MQTTClient` (paho) переподключается при разрыве сам, но подписки при этом не восстанавливает: сессия непостоянная (clean session), брокер её не помнит, а paho при реконнекте повторяет только незавершённые публикации. Поэтому после восстановления соединения `app.py` вызывает `bridge.republish()`, который заново подписывается на z2m-топики и WB-команды и перепубликует meta и контролы моста и всех известных устройств (с их последними значениями, но `available=offline` до подтверждения от zigbee2mqtt). Перепубликация безусловная: после сетевого разрыва брокер по-прежнему держит все retained-топики, а после перезапуска mosquitto (на WB он работает без persistence) они потеряны, и различить эти случаи клиент не может.
