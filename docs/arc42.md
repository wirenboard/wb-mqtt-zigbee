...
| `wb_converter/expose_mapper.py` | Маппинг z2m exposes → WB `ControlMeta` (12 numeric типов, binary, enum, text, range для writable с min/max); двуязычные `title` через `PROPERTY_TITLES` + `_localized_title`; перевод значений enum через `ENUM_VALUE_TITLES` |
