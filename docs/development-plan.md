...
1. Точное совпадение в курируемом двуязычном словаре `PROPERTY_TITLES` (en+ru, ~110 базовых property, живёт в `translations.py`) — например `temperature` → `{"en": "Temperature", "ru": "Температура"}`.
