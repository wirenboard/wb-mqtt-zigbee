...
Значения enum-контролов переводятся таблицей `ENUM_VALUE_TITLES`: ключ — **свойство**, затем само значение (`switch_type` → `rocker` → `{"en": "Rocker", "ru": "Клавишный"}`). Ключом выбрано свойство, а не значение: одно и то же слово требует разной формы в русском — `low` это «Низкая» при «Чувствительности», но «Низкий» при «Уровне», — а `off`/`toggle`/`previous` в разных свойствах значат разное. Промах по ключу даёт английскую подпись, то есть прежнее поведение, а не неверный перевод. Варианты с суффиксом эндпоинта (`switch_type_1`) берут подписи базового свойства.
