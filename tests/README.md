...
| Ошибки загрузки | `TestLoadConfigErrors` | Несуществующий файл, невалидный JSON, неверный корневой тип, отсутствующие обязательные поля, неверные строковые значения, `command_debounce_sec` и URL брокера отклоняются. |
