...
└── integration/                # интеграционные тесты (с моком MQTT-брокера)
    ...
    │   └── client.py           # FakeMqttClient (потоковый клиент wb-common без потока)
...
Расположены в `tests/integration/`. Заменяют MQTT-брокер на in-process `FakeMqttBroker` (без сети, без потоков, без Docker) — пригодно для запуска в Jenkins без специального окружения. `FakeMqttClient` — drop-in замена `wb_common.mqtt_client.MQTTClient` (потокового клиента) с подмножеством API, которое реально использует прод-код: `subscribe`, `unsubscribe`, `publish`, `message_callback_add/remove`, атрибутные коллбэки `on_connect`/`on_disconnect`, `start`/`wait_for_connection`/`is_connected`/`disconnect`/`stop`. Сетевого потока нет — его роль играет сам тест: CONNACK это `connect(rc=...)`, обрыв — `lose_connection()`. `publish()` возвращает `FakeMessageInfo` (аналог `MQTTMessageInfo` paho: `rc`, `mid`, `wait_for_publish(timeout)`, `is_published()`); подтверждения брокера приходят в `wait_for_publish()` (если тест не выставил `broker_acknowledges = False` — тогда ожидание «истекает») и в `stop()`, который, как поток paho перед выходом, досылает всю очередь и запоминает в `stopped_with_unacknowledged`, оставался ли в ней кто-то, кого прод-код не дождался сам. Вызовы `start`/`disconnect`/`stop` записываются по порядку в `calls`, аргумент `retry_first_connection` — отдельно.
...
| Reconnect | `TestReconnectFlow` | `republish()` инкрементит `Reconnects`; `set_all_unavailable()` переводит все известные устройства в `available=0`; `republish()` возвращает последние значения контролов, но оставляет `available=0` и ошибку до свежих данных от z2m; `remove_all()` снимает все retained-топики бриджа и устройств. |

### `tests/integration/test_app_lifecycle.py`

Проверяют [`wb/mqtt_zigbee/app.py`](../wb/mqtt_zigbee/app.py) — класс `WbZigbee2Mqtt`, который держит MQTT-соединение и переводит коллбэки `on_connect`/`on_disconnect` в действия над `Bridge`. В проде MQTT-цикл идёт в сетевом потоке paho, главный поток только ждёт запроса остановки; в тестах роль потока paho играет сам тест. Тесты подменяют конструктор `MQTTClient` через `monkeypatch.setattr`, чтобы получить `FakeMqttClient` (фабрика заодно проверяет, что запрошен потоковый клиент), и подменяют `signal.signal` на no-op (иначе модификация сигналов мешает pytest). Соединение/разрыв триггерится через `FakeMqttClient.connect(rc=...)` и `lose_connection()`. Тест, который зовёт `run()`, заранее запрашивает остановку — вызывает обработчик сигнала приложения напрямую или из побочного эффекта `wait_for_connection()` (то, что поток paho доставил, пока главный ждал соединения), иначе `run()` заблокировался бы навсегда. Настоящие сигналы и потоки не используются.
...
| Reconnect | `TestReconnect` | `connect(rc=0)` → `lose_connection()` → `connect(rc=0)` дёргает `Bridge.republish()`, инкрементит `Reconnects`; после регистрации устройств `lose_connection()` помечает все известные устройства `available = "0"`. |
| Connect failure modes | `TestConnectFailureModes` | `connect(rc=4/5)` (отказ авторизации) из `on_connect` только запрашивает остановку; клиента останавливает главный поток, один раз (`calls == ["start", "stop"]`), `run()` возвращает `EXIT_INVALIDARGUMENT` (2), ничего не публикуется, подписок нет, снятия топиков и строки «cannot be removed» в журнале нет — и при первом подключении, и после реконнекта; `connect(rc=1)` (другие коды) не приводит ни к `subscribe`, ни к запросу остановки, paho повторяет подключение; исключение из `Bridge.subscribe()` внутри `on_connect` (поток paho) доходит до `run()` как `EXIT_FAILURE` с трейсбеком в журнале, клиент остановлен. |
| Stop | `TestStop` | Обработчик сигнала сам ничего не публикует; после него `run()` в главном потоке снимает retained-топики каждого устройства и бриджа ровно по одному разу (устройство, найденное и стартовым сканом, не снимается второй раз; устройство, зарегистрированное после окончания скана, снимается по списку известных), ждёт последнее снятие с таймаутом `REMOVAL_TIMEOUT_S`, зовёт `stop()` только после подтверждения брокера и возвращает 0; чистый DISCONNECT от `stop()` ничего не переиздаёт. Призрак прошлого запуска, не вычищенный за отсутствием `bridge/devices`, снимается тоже. Трафик z2m (состояние бриджа, state/availability устройства, список устройств, счётчики статистики), пришедший после начала снятия, не переиздаёт ничего. Если брокер не подтверждает снятие (`broker_acknowledges = False`), пишется ошибка и `disconnect()` вызывается до `stop()`. Сигнал без брокера пишет одну ошибку в журнал, ничего не публикует и тоже даёт 0. |
| Run | `TestRun` | Боевая последовательность целиком: `start(retry_first_connection=True)` возвращается до CONNACK, соединение, регистрация устройств и сигнал происходят, пока главный поток ждёт, и `run()` не возвращается, пока снятие и `stop()` не выполнены (страховка от «условие перед циклом всегда ложно»); исключение из `start()` даёт `EXIT_FAILURE`, клиент всё равно остановлен. |
