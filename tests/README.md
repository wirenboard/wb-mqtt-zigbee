...
| `_make_enum` | `TestMakeEnum`, `TestEnumValueTitlesTable` | Конвенционная форма `{значение: {en, ru}}`: курируемое свойство переводится из `ENUM_VALUE_TITLES`; некурируемое значение → только `en`: многословное приводится к Title Case (`battery_full` → `Battery Full`), одиночное (`usb`, `heat`) и не-нижний регистр (`ON`, `2000K`) остаются дословно; числовые значения приводятся к строке; суффикс эндпоинта (`switch_type_1`) берёт подписи базового свойства; возвращаемые словари — копии; пустой список → `None`. Инварианты таблицы: у каждого значения непустой `en`, ключи без суффикса, у каждого — заголовок в `PROPERTY_TITLES`. |
