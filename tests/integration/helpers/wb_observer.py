...
class WbObserver:
    ...
    def messages_on(self, topic: str) -> list[PublishedMessage]:
        return [m for m in self._broker.publish_log if m.topic == topic]

    def messages_under(self, prefix: str) -> list[PublishedMessage]:
        return [m for m in self._broker.publish_log if m.topic.startswith(prefix)]

    def topics(self) -> list[str]:
        return [m.topic for m in self._broker.publish_log]
    ...
    def last_payload_on(self, topic: str) -> Optional[str]:
        m = self.last_on(topic)
        return None if m is None else m.payload.decode("utf-8")
    ...
    def has_publish_on(self, topic: str) -> bool:
        return any(m.topic == topic for m in self._broker.publish_log)
