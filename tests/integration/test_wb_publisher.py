...
def test_publish_bridge_device_publishes_all_bridge_controls(
    fake_mqtt_client: FakeMqttClient,
    wb_observer: WbObserver,
) -> None:
    _make_driver(fake_mqtt_client).publish_bridge_device()
...
def test_publish_bridge_device_initial_value_is_blank(
    fake_mqtt_client: FakeMqttClient,
    wb_observer: WbObserver,
) -> None:
    _make_driver(fake_mqtt_client).publish_bridge_device()
...
def test_publish_device_with_initial_values_uses_them(
    fake_mqtt_client: FakeMqttClient,
    wb_observer: WbObserver,
) -> None:
    driver = _make_driver(fake_mqtt_client)
...
def test_publish_device_without_initial_values_uses_blank(
    fake_mqtt_client: FakeMqttClient,
    wb_observer: WbObserver,
) -> None:
    driver = _make_driver(fake_mqtt_client)
...
def test_publish_device_clears_legacy_meta_subtopics(
    fake_mqtt_client: FakeMqttClient,
    wb_observer: WbObserver,
) -> None:
    driver = _make_driver(fake_mqtt_client)
...
def test_device_command_dispatches_control_id_and_value(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
...
def test_unsubscribe_device_commands_removes_only_writable(
    fake_mqtt_client: FakeMqttClient,
) -> None:
    controls = _sample_controls()
    driver = _make_driver(fake_mqtt_client)
...
def test_command_handler_does_not_fire_after_unsubscribe(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
    received: list[tuple[str, str]] = []
...
def test_retained_scan_collects_only_our_driver_devices(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
...
def test_retained_scan_excludes_bridge_device(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
...
def test_retained_scan_collects_per_device_controls(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
    fake_broker.inject(f"{DEVICES_PREFIX}/0xours/meta", _our_device_meta(), retain=True)
...
def test_retained_scan_ignores_invalid_meta_payloads(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
...
def test_stop_retained_scan_unsubscribes_wildcards(
    fake_mqtt_client: FakeMqttClient,
) -> None:
    driver = _make_driver(fake_mqtt_client)
    driver.start_retained_scan()
...
def test_start_retained_scan_resets_previous_state(
    fake_mqtt_client: FakeMqttClient,
    fake_broker: FakeMqttBroker,
) -> None:
